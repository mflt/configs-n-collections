...
import { google, Auth as _GoogleApisAuth, script_v1 as _GoogleScriptSchema } from 'googleapis'
import * as prompt from '@clack/prompts'
import color from 'picocolors'
import type { GcCollectionfromDoc_Config, DefaultEnvLiterals } from './types'

const _googleScriptApi = google.script('v1')
type _GoogleApisAuthClient = _GoogleApisAuth.OAuth2Client // ReturnType<typeof google.auth.fromJSON>
...
export async function retrieveCollectionfromDoc <
...
> (
  ...
  env: Envs,
  config: GcCollectionfromDoc_Config<CollectionLabels, Envs>
) {
  try {
    const authClient = await authorizedAuthClient(config as unknown as GcCollectionfromDoc_Config<['default'], Envs>)
    google.options({ auth: authClient })
    await retrieveRawMdDocsAndSave<CollectionLabels,Envs>(collectionLabels, env, config)
    return 0
  } catch (err) {
...
async function retrieveRawMdDocsAndSave <
  CollectionLabels extends GcCollectionfromDoc_Config<string[]>['namespace']['validCollectionLabels'],
  Envs extends string = DefaultEnvLiterals // @TODO string literal type
> (
  collectionLabels: Set<string>, // assumed to come to this point as a valid collectionLabels collection
  env: Envs,
  config: GcCollectionfromDoc_Config<CollectionLabels, Envs>
  // @TODO type
) {
      ...
      try {
        ...
        const gaxiosResponse = await _googleScriptApi.scripts?.run?.({
          ...
          requestBody: {
            ...
            parameters: [
              config.googleDrive.sources[env][
                collectionLabel as CollectionLabels[number]
              ]
            ]
            // devMode: true
            // sessionState: "my_sessionState"
          }
        }) satisfies _GoogleApisAuth.gaxios.GaxiosResponse<_GoogleScriptSchema.Schema$Operation>
...
async function authorizedAuthClient (
  config: GcCollectionfromDoc_Config
) {
  let googleApisAuthClient: _GoogleApisAuthClient | null =
    await loadSavedGoogleCredentialsIfExists(config)
  ...
  try {
    ...
    prompt.log.warn(`${color.bgYellowBright(color.black(
        'Likely now you\'ll be redirected to a google service to download one.'
      ))}`
    )
    prompt.log.message(`${color.bgYellowBright(color.red(
        'SECURITY WARNING:'
      ))}` + `${color.reset(color.bold(
        ' Give your consent carefully, you are blessing your/our script with medium risk rights, like:\n'
      ))}` +
    ...
    }) as ReturnType<typeof loadSavedGoogleCredentialsIfExists>
    ...
    googleApisAuthClient = resultfromAuthenticateOrCancel as typeof googleApisAuthClient
    prompt.log.success(
      `We got results from browser:\n` +
...
async function testifClientSecretsExists (  // errors are thrown and now end in overall failure
  ...
  try {
    const _clientSecrets = await Bun.file(
      config.repo.googleApis.clientConfigurationJsonPath  // @TODO test if usable
    ).json()
    return true
  } catch (err) {
    prompt.log.error(
      `Google Apis Client Secret/Id (aka configuration) file does not exist or is not a proper json.\n` +
      `a) Check if the file exists. Per the root configuration (root.config.toml) it's supposed to be located at:\n` +
      `${color.bgWhite(color.black(
        config.repo.googleApis.clientConfigurationJsonPath
      ))}\n` +
      `b) Retrieve the 'client configuration' file from your developers' console.\n` +
      `${color.bgBlack(color.blueBright(
        'Follow the below link (choose Desktop app as caller type):'
      ))}\n` +
...
async function loadSavedGoogleCredentialsIfExists ( // does not throw and can go further and acquire the missing file automatically
  ...
  try {
    const tokenCredentials = await Bun.file(
      config.repo.googleApis.tokenJsonPath  // @TODO test if usable
    ).json()
    return google.auth.fromJSON(tokenCredentials) as _GoogleApisAuthClient
  } catch (err) {
    prompt.log.warn(
      'Unable to load Google Apis Token credentials file (which is normally autogenerated)\n' +
      `from the location defined in the root configuration (root.config.toml), currently:\n` +
      `${color.bgWhite(color.black(
        config.repo.googleApis.tokenJsonPath
      ))}\n` +
      `${err}`
    )
    return null
...
async function saveGoogleCredentials (
  authClient: _GoogleApisAuthClient,
  config: GcCollectionfromDoc_Config
) {
  const _credentials = await Bun.file(
    config.repo.googleApis.clientConfigurationJsonPath
  ).json() // @TODO type
  ...
  try {
    await Bun.write(
      config.repo.googleApis.tokenJsonPath, pyl
    )
  } catch (err) {
